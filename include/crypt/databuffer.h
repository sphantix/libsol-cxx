        ...
        class CDataBuffer
        {
            ...
            public:
                ...
                void Shrink()
                {
                    ...
                    unsigned char *newbuf = (unsigned char*) std::malloc(MAX_BUFFER_SIZE);
                    assert(newbuf != NULL);

                    if (dlen > 0)
                    {
                        std::memcpy(newbuf, _pdata, dlen);
                    }
                    free(_pstart);
                ...
                {
                    n = htons(n);
                    std::memcpy(_pstart + offset, &n, 2);
                }

                void fill_int32(uint32_t n, int offset)
                {
                    n = htonl(n);
                    std::memcpy(_pstart + offset, &n, 4);
                }

                void fill_block(const void *src, int len, int offset)
                {
                    std::memcpy(_pstart + offset, src, len);
                }
            ...
            public:
                ...
                inline void Expand(int need)
                {
                    if (_pstart == NULL)
                    {
                        ...
                        while (len < need)
                            len <<= 1;
                        _pfree = _pdata = _pstart = (unsigned char*) std::malloc(len);
                        _pend = _pstart + len;
                    }
                    else if (_pend - _pfree < need)
                    {
                        ...
                        if (flen < need || flen * 4 < dlen)
                        {
                            ...
                            unsigned char *newbuf = (unsigned char *) std::malloc(bufsize);
                            if (newbuf == NULL)
                            {
